...
LC_NUMERIC=C awk -F';' 'NR>1 {distance[$6]+=$5} END {for (driver in distance) printf "%.3f;%s\n", distance[driver], driver}' data.csv | LC_NUMERIC=C sort -t';' -rnk1,1 | head -n 10 > temp/temp_d2.csv #sorts the total distance covered by each driver in descending order, and keeps the 10 highest values
